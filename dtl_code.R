...
##### Code for finding a two-stage drop-the-losers design with given FWER and
##### power, assuming an intermediate outcome in stage 1 and a definitive
##### outcome in stage 2

library(tidyverse)

# variance() returns the covariance matrix of the test statistics
variance   <- function(theta = 0.6, rho = 0.5) {
  ratio <- (theta/3)/(theta/3 + (1 - theta)/2)
  CovZ  <- diag(4)
  CovZ[1, 2] <- CovZ[2, 1] <- CovZ[3, 4] <- CovZ[4, 3] <- 0.5
  CovZ[1, 3] <- CovZ[2, 4] <- CovZ[3, 1] <- CovZ[4, 2] <- rho*sqrt(ratio)
  CovZ[1, 4] <- CovZ[2, 3] <- CovZ[4, 1] <- CovZ[3, 2] <- 0.5*rho*sqrt(ratio)
  CovZ
}

# returns the A matrix corresponding to either rejecting H1 or H2
get_A      <- function(k = 1) {
  if (k == 1) {
    rbind(c(1, -1, 0, 0),
          c(0,  0, 1, 0))
...
# fwer_hg() finds the difference between the FWER of a given design and a target
# FWER under HG
fwer_hg    <- function(c, alpha = 0.05, theta = 0.6, rho = 0.5) {
  CovZ <- variance(theta = theta, rho = rho)
  A1   <- get_A()
  2*mvtnorm::pmvnorm(lower = c(0, c),
                     upper = rep(Inf, 2),
                     mean  = numeric(2),
                     sigma = A1%*%CovZ%*%t(A1))[1] - alpha
}

# fwer() returns the value of the FWER function
fwer       <- function(delta1I, delta2I, delta1D, delta2D, rho, theta, N, c) {
  N1     <- theta*N/3
  N2     <- N1 + N*(1 - theta)/2
  fwer   <- 0
  EZ     <- c(delta1I, delta2I, delta1D, delta2D)*sqrt(c(N1, N1, N2, N2)/2)
  CovZ   <- variance(theta, rho)
  if (delta1D <= 0) {
    A1   <- get_A()
    fwer <- mvtnorm::pmvnorm(lower = c(0, c),
                             upper = c(Inf, Inf),
                             mean  = as.double(A1%*%EZ),
                             sigma = A1%*%CovZ%*%t(A1))[1]
  }
  if (delta2D <= 0) {
    A2   <- get_A(k = 2)
    fwer <- fwer + mvtnorm::pmvnorm(lower = c(0, c),
                                    upper = c(Inf, Inf),
                                    mean  = as.double(A2%*%EZ),
                                    sigma = A2%*%CovZ%*%t(A2))[1]
  }
  fwer
}

# power_lfc() finds the difference between the power function P1 under the LFC
# of a given design and a target power
power_lfc  <- function(N, beta = 0.1, c, theta, rho, deltaD1, deltaI1, deltaD0,
                       deltaI0) {
  N1   <- theta*N/3
  N2   <- N1 + N*(1 - theta)/2
  A1   <- get_A()
  EZ   <- c(deltaI1, deltaI0, deltaD1, deltaD0)*sqrt(c(N1, N1, N2, N2)/2)
  CovZ <- variance(theta, rho)
  mvtnorm::pmvnorm(lower = c(0, c),
                   upper = c(Inf, Inf),
                   mean  = as.double(A1%*%EZ),
                   sigma = A1%*%CovZ%*%t(A1))[1] - (1 - beta)
}

# finddesign() returns a design with specified family-wise error rate and power.
# Arguments:
# deltaD1      - standardised clinically relevant difference for power
#                (definitive outcome)
# deltaI1      - standardised clinically relevant difference for power
#                (intermediate outcome)
# deltaD0      - standardised uninteresting treatment threshold for power
#                (definitive outcome)
# deltaI0      - standardised uninteresting treatment threshold for power
#                (intermediate outcome)
# alpha        - required family-wise error rate under the global null
# beta         - one minus required power under the least favourable
#                configuration
# theta        - proportion of patients recruited in stage 1
# rho          - the correlation between the intermediate and definitive
#                outcomes
# fwer_control - whether the FWER control requirement assumes rho is known
# Output: A list with entries c and N. c is the required critical value and N is
# the total sample size used by the design
finddesign <- function(
  deltaD1      = 0.545,
  deltaI1      = 0.545,
  deltaD0      = 0.178,
  ...
  theta        = 0.6,
  rho          = 0.5,
  fwer_control = "unknown")
{
  if (fwer_control == "known") {
    rho_fwer <- rho
  } else {
    rho_fwer <- 1
  }
  c          <- stats::uniroot(fwer_hg,
                               lower = -2,
                               upper = 5,
                               alpha = alpha,
                               theta = theta,
                               rho   = rho_fwer)$root
  N          <- stats::uniroot(power_lfc,
                               lower   = 0,
                               upper   = 2000,
                               beta    = beta,
                               c       = c,
...
##### Section 2.3.1: Impact of rho on required sample size #####################

scenarios <- expand.grid(rho          = seq(0, 1, 0.1),
                         fwer_control = c("known", "unknown"))
N         <- nrow(scenarios)
for (i in 1:nrow(scenarios)) {
  N[i]    <- finddesign(rho          = scenarios$rho[i],
                        fwer_control = scenarios$fwer_control[i])$N
}
df        <- tibble::as_tibble(cbind(scenarios, N))
ggplot(df, aes(rho, N, colour = fwer_control)) +
  geom_point() +
  geom_line() +
  theme_bw() +
  ...
  labs(x      = expression(rho),
       ...
       colour = expression(paste("FWER control treats ", rho, " as")))

##### Section 2.3.2: Optimal timing of the interim analysis ####################

scenarios <- expand.grid(rho          = c(0.25, 0.5, 0.75, 1),
                         theta        = seq(0.25, 0.9, 0.05),
                         fwer_control = c("known", "unknown"))
N         <- nrow(scenarios)
for (i in 1:nrow(scenarios)) {
  N[i]    <- finddesign(rho          = scenarios$rho[i],
...
df        <- tibble::as_tibble(cbind(scenarios, N))
df$rho    <- paste("rho ==", df$rho)
ggplot(df, aes(theta, N, colour = fwer_control)) +
  facet_grid(~rho, labeller = label_parsed) +
  geom_point() +
  geom_line() +
  theme_bw() +
  theme(legend.position = "bottom") +
...
##### Section 2.3.3: Family-wise error rate control ############################

scenarios <- expand.grid(delta1I = seq(-2*0.545, 2*0.545, length.out = 25),
                         delta2I = seq(-2*0.545, 2*0.545, length.out = 25),
                         rho     = c(0.25, 0.5, 0.75, 1))
FWER      <- nrow(scenarios)
for (i in 1:nrow(scenarios)) {
  FWER[i] <- fwer(delta1I = scenarios$delta1I[i],
                  delta2I = scenarios$delta2I[i],
                  delta1D = 0,
                  delta2D = 0,
                  rho     = scenarios$rho[i],
                  theta   = 0.6,
...
df        <- tibble::as_tibble(cbind(scenarios, FWER))
df$rho    <- paste("rho ==", df$rho)
ggplot(df, aes(delta1I, delta2I, fill = FWER)) +
  facet_grid(~rho, labeller = label_parsed) +
  geom_tile() +
  scale_fill_gradientn(colours = Turbo()) +
  theme_bw() +
  theme(aspect.ratio    = 1,
        legend.position = "bottom") +
  labs(x    = expression(delta["1I"]),
       y    = expression(delta["2I"]),
       fill = "FWER")
